...
Track Description
Most fans watching the World Cup are doing it with a phone in their hand. TxLINE gives you live scores, real-time odds, and match events for all 104 games, the kind of data that until now only the big operators could access. We want to see what builders who are also fans do with it. The experiences that could exist here have not been built yet.

Ideas to get started
Group Sweepstake: Friends are assigned World Cup teams. As matches play, the leaderboard updates live from TxLINE data rather than waiting for someone to update a spreadsheet.

AI Pundit Bot: A Telegram bot that sends a message every time something significant happens — a goal, a red card, a big odds shift — explaining what happened and what the market thinks now. Bonus points for TTS functionality.

Hi-Lo Stats Game: Will the next match stat — shots, corners, possession — be higher or lower than the last? Guess before each TxLINE update, build a streak, share your score. Replayable across 104 games.

Judging Criteria
Fan Accessibility & UX: Is the product engaging, intuitive, and polished enough that a mainstream, non-technical sports fan would regularly open and use it?

Real-Time Responsiveness: Does the application dynamically respond to and update fluidly based on what is actively unfolding on the pitch?

Originality & Value Creation: Does the project introduce a genuinely new consumer experience or fan interaction model, rather than just repackaging existing sports feeds?

Commercial & Monetization Path: Is there a clear, viable product utility or potential business and monetization model behind the application?

Completeness & Execution: Does the submission feel like a functional, complete end-to-end product feature, even if the overall technical scope is deliberately small?
...
A live (mainnet or devnet) product that works during a match. We are focused on clarity of use case and quality of execution over scope.
...
Must use TxLINE data as a live input and sign up through Solana.

Products must be functional, not mockups.
...
Application Access: A working link to your deployed website OR a functional API endpoint for judges to test.
...
Initial Review & Shortlisting: Following the close of submissions at the July 19, 2026, 23:59 UTC, the judges will review entries and compile a shortlist of top-performing teams.
